...
import { createExperimentForDemo } from "@fpllm/db";
...
export async function POST(request: Request) {
  ...
  try {
    const experiment = await createExperimentForDemo(parsed.data.submissionId);
    return NextResponse.json({ ok: true, experiment }, { status: 201 });
  } catch (error) {
    return NextResponse.json({ ok: false, code: "EXPERIMENT_CREATE_FAILED", message: error instanceof Error ? error.message : "Unknown error" }, { status: 409 });
  }
}
