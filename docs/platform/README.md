...
16. `PRODUCTION_PROVIDER_DECISIONS_v0.5.md` — linked Platform v0.5 P0 decision record: AWS `eu-west-1`, ECS Express/Fargate web, RDS PostgreSQL 18.6, EC2 + gVisor worker/sandbox, ECR, CloudWatch/X-Ray, Secrets Manager, AWS-managed production origin/TLS, restore targets, and release/rollback process. P0 is closed by this record; P1 implementation and verification remain open.
17. `GITHUB_PROVIDER_LIMITS_v0.5.md` — P0 companion record for GitHub as an external production provider: published OAuth/REST/GitHub-App limits, current request shape, the initial beta source-materialization envelope, and required provider-aware throttling/defer behavior before P2/P3.

Decision precedence is: v1.2 delta over v1.1 where they conflict; otherwise v1.1 remains in force. Within v1.2, the post-review qualification in §8 governs evidence claims. `PRODUCTION_PROVIDER_DECISIONS_v0.5.md` together with `GITHUB_PROVIDER_LIMITS_v0.5.md` forms the linked P0 provider/operations record authorized by the v0.5 contract and supersedes only v1.2 §1.4 item 4's pre-P0 statement that production provider selection was still open. Runtime subcontracts may narrow implementation details but must not weaken frozen security or evidence invariants.
