...
2. `../../PROJECT_DECISIONS_v1.2.md` — authoritative v1.2 delta; supersedes conflicting v1.1 decisions until the next full consolidation.
...
14. `REAL_LEARNER_IDENTITY_v0.4.md` — verified real learner identity, repository onboarding, environment qualification, and no-seed submission/status lifecycle.
...
Decision precedence is: v1.2 delta over v1.1 where they conflict; otherwise v1.1 remains in force. Runtime subcontracts may narrow implementation details but must not weaken frozen security or evidence invariants.
