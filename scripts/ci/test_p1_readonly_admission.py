...
def passing_observation() -> dict[str, object]:
    return {
        ...
        "identity": {"Account": "123456789012", "Arn": "arn:aws:iam::123456789012:role/read-only"},
        "region": {"RegionName": "eu-west-1", "OptInStatus": "opt-in-not-required"},
        ...
        "quotas": {
            ...
            "ec2": [quota("Running On-Demand Standard (A, C, D, H, I, M, R, T, Z) instances", 16)],
            "rds": [
                quota("DB instances", 40),
                quota("Total storage for all DB instances", 100000),
                quota("Reserved DB instances", 40),
            ],
            "elasticloadbalancing": [quota("Application Load Balancers per Region", 50)],
            "vpc": [
            ...
            "dynamodb": [quota("Tables per Region", 2500)],
        },
        "usage": {
            "vpcs": 1,
...
class AdmissionTests(unittest.TestCase):
    def evaluate(self, observed: dict[str, object], *, require_kms: bool = False):
        return module.evaluate(observed, ["10.42.16.0/24", "10.42.17.0/24"], require_kms)
    ...
    def test_small_private_subnets_fail_capacity_margin(self) -> None:
        checks, summary = module.evaluate(
            passing_observation(),
        ...
        self.assertEqual(summary["status"], "FAIL")
        failed = {check.id for check in checks if check.status == "FAIL"}
        self.assertIn("topology.private-control-subnet-ip-capacity", failed)
    ...
    def test_allowlist_contains_only_descriptive_operations(self) -> None:
        forbidden_prefixes = ("create", "delete", "put", "update", "modify", "run", "start", "stop", "register", "deregister")
