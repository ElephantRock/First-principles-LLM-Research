...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, positions: torch.Tensor | None = None) -> torch.Tensor:
        ...
        q = _rope(q, positions, self.rope_theta).reshape(b, hkv, groups, s, dh)
        k = _rope(k, positions, self.rope_theta).repeat_interleave(groups, dim=1).reshape(b, hkv, groups, s, dh)
        v = v.repeat_interleave(groups, dim=1).reshape(b, hkv, groups, s, dh)
